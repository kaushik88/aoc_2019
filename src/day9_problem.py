...
def get_token_index(token_digits, digit_index, tokens, token_index, rb):
    token_mode = safe_get_mode(token_digits, digit_index)
    token_value = token_index
    if token_mode != 1:
        token_value = int(tokens[token_value])
        if token_mode == 2:
            token_value += rb
    return token_value

def get_outputs(tokens, input_value='2'):
    outputs = []
    i = 0
    rb = 0
    while i < len(tokens):
        ...
        first_index = get_token_index(token_digits, len(token_digits) - 3, tokens, i+1, rb)
        second_index = get_token_index(token_digits, len(token_digits) - 4, tokens, i+2, rb)
        third_index = get_token_index(token_digits, len(token_digits) - 5, tokens, i+3, rb)
        first_value = int(tokens[first_index]) if first_index < len(tokens) else 0
        second_value = int(tokens[second_index]) if second_index < len(tokens) else 0
        print("Token Digits", token_digits, "Position ", i, "First value", first_value, "Second Value", second_value, "Third_index", third_index)
        if last_token == 1 or last_token == 2:
            if last_token == 1:
                tokens[third_index] = str(first_value + second_value)
            else:
                tokens[third_index] = str(first_value * second_value)
            i = i+4
        elif last_token == 3:
            tokens[first_index] = input_value
            i += 2
        elif last_token == 4:
            print("Output at {} with {}".format(i, first_value))
            outputs.append(str(first_value))
            i = i + 2
        elif last_token == 5:
            if first_value != 0:
                i = second_value
            ...
            if first_value < second_value:
                tokens[third_index] = str(1)
            else:
                tokens[third_index] = str(0)
            i += 4
        elif last_token == 8:
            if first_value == second_value:
                tokens[third_index] = str(1)
            ...
            if len(token_digits) > 1:
                prev_token = token_digits[len(token_digits) - 2]
            else:
                prev_token = 0
            if prev_token == 9:
                break
            else:
                print("Old Relative Base", rb)
                rb += first_value
                print("New Relative Base is {} by adding value {}".format(rb, first_value))
                i += 2
    return outputs
